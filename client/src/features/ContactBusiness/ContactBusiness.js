...
import { useAddNewInvestorProfileMutation, useGetInvestorProfileByUserIDQuery, useAddNewProfileInvestorMapMutation } from '../../pages/InvestorProfilePage/InvestorSlice'
...
const CreateContact = ({ investorId }) => {
    const { userID } = useAuth()
    const businessId = useSelector(state => state.profile.businessId)
    const [addNewProfileInvestorMap, { isSuccess, isError, error }] = useAddNewProfileInvestorMapMutation()


    const handleContactBusinessClick = () => {
        addNewProfileInvestorMap({ businessId, investorId })
    }
    ...
    return (
        <div className='flex flex-col gap-2 self-start max-w-xs p-6 w-full border border-gray-200 bg-white shadow-[0_0_10px_0] shadow-gray-300'>
            ...
            <div>
                <form>
                    <div className='flex flex-col gap-2'>
                        <button type="submit" className='bg-[#ffc040] cursor-pointer rounded-3xl text-center font-bold px-5 py-3' onClick={handleContactBusinessClick}> Contact this Business </button>
                    </div>
                </form>
            </div>
