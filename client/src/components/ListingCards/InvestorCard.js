...
const InvestorCard = ({ item }) => {
    ...
    return (
        <div className='flex  flex-col w-full p-2 border border-gray-300 drop-shadow-md shadow-gray-300'>
            <p className='text-blue-400 text-base'>{item.designation} , {item?.industry.map(obj => obj.title).join(", ")}</p>
            <p className='text-sm'>
            ...
            <div className='flex gap-1 py-1'>
                ...
                <div >
                    {item?.investorImage[0] ? <Avatar className='rounded-md' src={item?.investorImage[0]} size="75" /> : <Avatar className='rounded-md' facebookId="100008343750912" size="75" />}
                </div>
            </div>
            ...
            <div className="flex py-2 divide-x-2 gap-2 items-center justify-end flex-wrap">
                ...
                <div className='px-2'>
                    <button className='px-4 rounded-md py-2 bg-[#f5d244] cursor-pointer' onClick={() => navigate(`/investors/investorProfile/${item._id}`)}>Contact Business</button>
                </div>
            </div>
        </div>
    )
