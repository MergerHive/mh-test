...
const BusinessCard = ({ item }) => {
    ...
    return (
        <div className='flex  flex-col w-full p-2 border border-gray-300 drop-shadow-md shadow-gray-300'>
            <p className='text-blue-400 text-base'>{item.companyName}</p>
            ...
            <div className='flex gap-2 py-1'>
                ...
                <div>
                    ...
                    <p className='text-sm'>{item?.businessDetails?.companyLocation[0]?.description}</p>
                </div>
            </div>
            ...
            <div className="flex py-2 divide-x-2 gap-2 items-center justify-end flex-wrap">
                ...
                <div className='px-2'>
                    <button className='px-4 rounded-md py-2 bg-[#f5d244] cursor-pointer' onClick={() => navigate(`/business-for-sale/businessProfile/${item._id}`)}>Contact Business</button>
                </div>
            </div>
        </div>
    )
