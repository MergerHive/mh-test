...
import { useAddNewInvestorProfileMutation, useGetInvestorProfileByUserIDQuery } from './InvestorSlice';
...
const CreateInvestorProfile = () => {
  ...
  const updateData = async (stage, event) => {
    ...
    event = { user: userID, intrestTypes, stage, isProfileActive: true, phNumber, ...event, investorImage: businessImageurls, investorFiles: investorFiles }
    ...
    <>
      {
        isLoading ? <MHLoadingOverlay /> :
          (data?.investorProfile ? <div>Investor Profile Present </div> : (<div className={`bg-gradient-radial bg-[length:36px_36px] ${styles.paddingX} ${styles.flexCenter} h-full flex-col`}>
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                        ...
                        {bpTypeList.map((option) => (
                          <div key={option.id} className="flex gap-2 items-center">
                            <input
                              type="checkbox"
                              value={option.value}
                              checked={intrestTypes.includes(option.value)}
                              onChange={(e) => {
                                if (e.target.checked) {
                                  setIntrestTypes([...intrestTypes, option.value]);
                                } else {
                                  setIntrestTypes(intrestTypes.filter((type) => type !== option.value));
                                }
                              }}
                              id={option.id}
                            />
                  ...
                  <InputSection title={"Confidential Personal Details"}>
                    <div className='grid grid-cols-1 items-center my-2 gap-2 md:grid-cols-2'>
                      ...
                      <Input id="fullName" {...register('fullName')} hasError={errors?.fullName} errorMsg={errors?.fullName?.message} label={'Your Name'} defaultValue={firstName + " ," + lastName} />
                      <Input id={'company'} {...register('company'
                      )} hasError={errors?.company} errorMsg={errors?.company?.message} label={'Company Name'} />
                    </div>
                  </InputSection>
                  <div>
                    <InputSection title={"Investor Profile Details"}>
                      <div className='grid grid-cols-1 items-center my-2 gap-2 md:grid-cols-2'>
                        ...
                        <Input id={'companyWebsite'} {...register('companyWebsite')} label={'Company Website'} />
                        <Controller
                          name="industry"
                          control={control}
                          defaultValue={[]}
                      ...
                      <div className='grid grid-cols-1 items-center my-2 gap-2 md:grid-cols-1'>
                        <TextArea id={'investCriteria'} {...register('investCriteria')} label={'Investment Criteria'} />
                        <TextArea id={'introMsg'} {...register('introMsg')} label={'Professional summary/experties'} />
                      </div>
                    </InputSection>
                  </div>
